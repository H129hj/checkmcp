...
export const LEARN: LearnPage[] = [
  ...
  },
  {
    ...
    "faq": [
      ...
      },
      {
        ...
        "a": "Prefer first-party or well-maintained open-source servers, then read the tool descriptions and input schemas yourself, looking for instructions aimed at the model, literal secrets in schemas, and unconfirmed destructive tools. Inventory the capability mix to make sure no single server (or your whole agent) forms the lethal trifecta, prefer local stdio servers for sensitive data, and scope credentials tightly. The fastest path is to run an automated audit like CheckMCP (uvx checkmcp <url>), which checks all of this and re-checks the server over time."
      },
      {
  ...
  },
  {
    ...
    "faq": [
      ...
      },
      {
        ...
        "a": "Audit the running server. CheckMCP probes your live endpoint and returns an explainable MCP Score /100 across seven pillars, runs an OWASP MCP Top 10 security pass (with hard caps for hardcoded secrets, critical injection and a failed handshake), and can run opt-in behavioral evals against read-only tools to catch tool-response poisoning and exfiltration. Run uvx checkmcp <url>, use checkmcp.dev, or add the GitHub Action to fail CI on a security or score regression."
      }
    ],
    "related": [
      "what-is-an-mcp-server",
  ...
  },
  {
    ...
    "faq": [
      ...
      },
      {
        ...
        "a": "Audit the live server before trusting it. CheckMCP probes the endpoint, inspects every tool, resource and prompt, and produces an explainable MCP Score (grade A-F) across seven pillars — grading tool design, schema and description quality, context cost and coverage, and running an OWASP MCP Top 10 security pass that flags poisoned descriptions, secrets in schemas, unsafe destructive tools and the lethal trifecta. A secret in a tool schema caps the grade at D and a failed handshake caps it at F. Run it with uvx checkmcp <url> or at checkmcp.dev."
      }
    ],
    "related": [
      "model-context-protocol",
  ...
  },
  {
    ...
    "faq": [
      ...
      },
      {
        ...
        "a": "Treat it like installing a dependency that runs with your credentials. Prefer official or first-party servers, pin a specific version, read the tool descriptions and schemas, and grant least-privilege tokens. Audit it first - for example run `uvx checkmcp <url>` or use checkmcp.dev to get a 0-100 MCP Score covering tool poisoning, hardcoded secrets, and the lethal trifecta - and re-vet on every update."
      }
    ],
    "related": [
      "what-is-an-mcp-server",
  ...
  },
  {
    ...
    "checkmcpRelation": "Before you run `claude mcp add` on a third-party server, CheckMCP tells you whether it is safe to wire into your agent. Paste the server's URL at checkmcp.dev — or run `uvx checkmcp <url>` from the open-source CLI (MIT, stdlib-only) — and CheckMCP probes the live endpoint and returns an explainable, vendor-neutral MCP Score 0-100 (grade A-F) across seven pillars led by security (weight 20), which runs an OWASP MCP Top 10 pass for tool poisoning, hardcoded secrets, command injection and the lethal trifecta. Hard caps surface the worst configurations immediately: a secret found in a tool schema caps the grade at D, and a failed MCP handshake caps it at F. Opt-in behavioral evals exercise read-only tools with canary inputs to catch prompt-injection and data-exfiltration in tool responses. For servers you depend on in a shared `.mcp.json`, the GitHub Action (`uses: H129hj/checkmcp@v1`) can gate CI on a score regression or rug-pull, and drift monitoring re-checks on every change; an in-band Gateway (passive or active mode) can block tool-poisoning and drift before they reach your agent. CheckMCP audits the server; you keep your Claude Code config clean.",
    "faq": [
      {
